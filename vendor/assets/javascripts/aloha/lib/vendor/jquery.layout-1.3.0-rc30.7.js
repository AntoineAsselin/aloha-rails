...
define('jquery-layout',['jquery'],function(jQuery){;(function($){var min=Math.min,max=Math.max,round=Math.floor,isStr=function(v){return $.type(v)==="string";},runPluginCallbacks=function(Instance,a_fn){if($.isArray(a_fn))
...
showMasks(pane);},onResizerLeave=function(evt,el){var e=el||this,pane=$(e).data("layoutEdge"),name=pane+"ResizerLeave";timer.clear(pane+"_openSlider");timer.clear(name);if(!el)
...
sName="fxName"+n,sSpeed="fxSpeed"+n,sSettings="fxSettings"+n,fxName=o[sName]=o[sName]||d[sName]||o.fxName||d.fxName||"none";if(fxName==="none"||!options.effects[fxName]||!$.effects||(!$.effects[fxName]&&!$.effects.effect[fxName]))
...
$P.hover(allowOverflow,resetOverflow);if(state.initialized){initHandles(pane);initHotkeys(pane);afterInitPane(pane);}},afterInitPane=function(pane){var $P=$Ps[pane],s=state[pane],o=options[pane];if(!$P)return;if($P.data("layout"))
...
$R.css(side,sC.inset[side]+getPaneSize(pane));});},initHandles=function(panes){panes=panes?panes.split(","):_c.borderPanes;$.each(panes,function(i,pane){var $P=$Ps[pane];$Rs[pane]=false;$Ts[pane]=false;if(!$P)return;var
...
$R.addClass(resizerClass+" "+resizerPaneClass);helperClassesSet=false;$('body').disableSelection();showMasks(pane);},drag:function(e,ui){if(!helperClassesSet){ui.helper.addClass(helperClass+" "+helperPaneClass).css({right:"auto",bottom:"auto"}).children().css("visibility","hidden");helperClassesSet=true;if(s.isSliding)$Ps[pane].css("zIndex",z.pane_sliding);}
...
showMasks(pane);}};},sizeMask=function(){var $M=$(this),pane=$M.data("layoutMask"),s=state[pane];if(s.tagName=="IFRAME"&&s.isVisible)
$M.css({top:s.offsetTop,left:s.offsetLeft,width:s.outerWidth,height:s.outerHeight});},sizeMasks=function(){$Ms.each(sizeMask);},showMasks=function(pane,args){var c=_c[pane],panes=["center"],z=options.zIndexes,a=$.extend({objectsOnly:false,forSliding:state[pane].isSliding},args),o,s;if(a.forSliding)
...
if(!skipCallback&&(state.initialized||o.triggerEventsOnLoad)){if(!isShowing)_runCallbacks("onclose_end",pane);if(isShowing)_runCallbacks("onshow_end",pane);if(isHiding)_runCallbacks("onhide_end",pane);}}},setAsClosed=function(pane){var
...
close(pane);};},slideToggle=function(evt_or_pane){var pane=evtPane.call(this,evt_or_pane);toggle(pane,true);},lockPaneForFX=function(pane,doLock){var $P=$Ps[pane],s=state[pane],o=options[pane],z=options.zIndexes;if(doLock){showMasks(pane,{objectsOnly:true});$P.css({zIndex:z.pane_animate});if(pane=="south")
...
$.extend(s,cssMinDims(pane),{maxWidth:newCenter.width,maxHeight:newCenter.height});CSS=newCenter;CSS.width=cssW($P,CSS.width);CSS.height=cssH($P,CSS.height);hasRoom=CSS.width>=0&&CSS.height>=0;if(!state.initialized&&o.minWidth>newCenter.width){var
...
CSS.top=newCenter.top;CSS.bottom=newCenter.bottom;CSS.height=cssH($P,newCenter.height);s.maxHeight=CSS.height;hasRoom=(s.maxHeight>=0);if(!hasRoom)s.noVerticalRoom=true;}
...
makePaneFit(pane);$P.css(visCSS);if(!s.isVisible)
...
$.extend(sC,elDims($N,options.inset));if(!sC.outerHeight)return;if(evt_or_refresh===true)
setPanePosition();if(false===_runCallbacks("onresizeall_start"))return false;var
...
return null;else
return Instance;}
$(function(){var b=$.layout.browser;if(b.msie)b.boxModel=$.support.boxModel;});})(jQuery);(function($){if(!$.ui)$.ui={};$.ui.cookie={acceptsCookies:!!navigator.cookieEnabled,read:function(name){var
